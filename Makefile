...
help-aseprite: ## Outputs compile-aseprite help screen.
	@$(DOCKER_COMP) run --rm bash bash /project/compile.sh --help
...
clean: ## Remove Aseprite build artifacts.
	@$(DOCKER_COMP) run --rm bash rm -rf /project/output/aseprite/build
...
dist-clean-aseprite: ## Remove Aseprite build artifacts & project.
	@$(DOCKER_COMP) run --rm bash rm -rf /project/output/aseprite
